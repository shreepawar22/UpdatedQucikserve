...
export const saveOrderToHistory = (orderData) => {
  ...
  // Add new order to beginning of array (most recent first)
...
const OrdersHistory = () => {
  ...
  const loadData = () => {
    ...
    try {
      ...
      // Sort orders by date (newest first) and filter orders with userDetails
      const sortedOrders = storedOrders
        .filter(order => order && order.userDetails)
  ...
  const handleDownloadReceipt = (order) => {
    ...
    const receiptContent = `
      <html>
        <head>
          <title>Receipt #${order.id || order.orderDate}</title>
          <style>
        ...
        {isLoading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {filteredOrders.map((order) => {
              ...
              return (
                <div key={order.id || order.orderDate} className="bg-white rounded-lg shadow-sm overflow-hidden hover:shadow-md transition-shadow relative">
                  <button
